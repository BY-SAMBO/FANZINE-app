...
} from "@/lib/hooks/use-pos-v2";
import { OrderItemRow } from "@/components/pos-v2/order/order-item";
import {
  MapPin,
  User,
  Phone,
...
export function RemoteOrderPanel() {
  ...
  const delivery = usePosV2Store((s) => s.delivery);
  const grandTotal = usePosV2Store((s) => s.grandTotal);
  const removeItem = usePosV2Store((s) => s.removeItem);
  const updateItemQuantity = usePosV2Store((s) => s.updateItemQuantity);
  const clearOrder = usePosV2Store((s) => s.clearOrder);
  const setLocation = usePosV2Store((s) => s.setLocation);
  const setCustomerName = usePosV2Store((s) => s.setCustomerName);
  const setCustomerPhone = usePosV2Store((s) => s.setCustomerPhone);
  const setComment = usePosV2Store((s) => s.setComment);
  ...
  const deliveryFee = delivery.location?.delivery_fee || 0;

  const canSubmit =
    order.items.length > 0 &&
    delivery.location &&
    delivery.customer_name.trim().length > 0;
  ...
  const handleSubmit = async () => {
    if (!canSubmit || !delivery.location) return;

    try {
      const result = await submitOrder.mutateAsync({
        location_id: delivery.location.id,
        items: order.items,
        customer_name: delivery.customer_name.trim(),
        customer_phone: delivery.customer_phone.trim(),
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-[380px] max-h-[80vh] overflow-y-auto">
            ...
            <div className="p-5 space-y-3">
              ...
              <div className="flex items-start gap-2">
                ...
                <div>
                  <p className="text-sm font-bold text-gray-900">{delivery.location?.name}</p>
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-semibold text-gray-900">{delivery.customer_name}</span>
                {delivery.customer_phone && (
                  <span className="text-xs text-gray-400 ml-1">{delivery.customer_phone}</span>
                )}
              </div>
